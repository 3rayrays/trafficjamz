...
for n=2:(numIterations+1)
    ...
    for a=1:length(currentCars)
        ...
        if (car(i).frustration(end)>=frustrationThreshold || ...
            ...
            laneChange = canChangeLanes(i, currentPositions, ...
                numLanes,minFollowingDistance);
...
for a = 1:length(posnmatrix(1,:))
    ...
    for dt = 1:length(car)
        ...
        %carposn(dt) = scatter(posnmatrix(dt,a), lanematrix(dt,a),100,...
        %    'filled','s','MarkerEdgeColor','black','MarkerFaceColor',...
        %    colors{carIndex},'LineWidth',1.5);

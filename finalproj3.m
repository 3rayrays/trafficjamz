...
for n=2:(numIterations+1)
    ...
    if n==2 || length(currentPositions(:,1))<numberOfCars
        ...
        currentPositions = [currentPositions; index car(index).position(end) car(index).speed(end) car(index).lane];
    ...
    for a=1:length(currentCars)
        ...
        if car(i).position(end)>=roadLength
            ...
            currentPositions(currentPositions(:,1)==i,4)=car(i).lane(end);
        end
    end
end

...
avgfrustrationRunNoHonking = [];


for runs=1:50
    clearvars -except avgfrustrationRunHonking ... 
        avgfrustrationRunNoHonking 
    ...
    avgfrustrations = [];
    for i=1:index
        avgfrustrations = [avgfrustrations, mean(car(i).frustration)];
    end
    
    avgfrustrationRunHonking(end+1) = mean(avgfrustrations);
...
for runs=1:50
    clearvars -except avgfrustrationRunHonking avgfrustrationRunNoHonking
    %% simulation constants
    dt = 0.1;
    % simulation length
    simLength = 50;
    ...
    for n=2:(numIterations+1)
        ...
        for a=1:length(currentCars)
            ...
            if (car(i).frustration(end)>=frustrationThreshold || ...
                ...
                % check whether you are able to change lanes
                laneChange = canChangeLanes(i, currentPositions, numLanes,minFollowingDistance);
                % if you can't change lanes, stay in your lane & be
                % frustrated
                if laneChange == 0
                    %car(i).honk(end) = 1;
                    % frustration is relieved from honking
...
%                   end
    ...
    avgfrustrationRunNoHonking(end+1) = mean(avgfrustrations);
    
end
%%
mean(avgfrustrationRunHonking)
mean(avgfrustrationRunNoHonking)

honkstatus = [ones(1,50), zeros(1,50)];
avgfrustration = [avgfrustrationRunHonking, avgfrustrationRunNoHonking];

%%
% display 
figure;
hold on;
boxplot(avgfrustration,honkstatus,'Labels',{'No Honking','Honking'});
title('Average frustration level by honking status');
ylabel('Average frustration level');
xlabel('Whether honking is implemented in the model');
%ylim([1 2]);
hold off;
